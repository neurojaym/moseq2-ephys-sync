...
def avi_workflow(base_path, save_path, num_leds=4, led_blink_interval=5, led_loc=None, avi_chunk_size=1000, overwrite_extraction=False):
    ...
    # Load timestamps
    dev_timestamps = np.load(timestamp_path)
    match_ts = np.load(timestamp_matches) #auto generated by acquisition code, first col is bottom cam, second col is top cam on MD rig

    timestamps = dev_timestamps[match_ts.T[1]]
    ...
    frame_batches = gen_batch_sequence(n_timestamps, avi_chunk_size, 0,0)
    ...
    if not os.path.isfile(avi_led_events_path) or overwrite_extraction:
        ...
        for i in tqdm(range(num_chunks)[0:]):
            ...
            for j, frame_num in enumerate(frame_batches[i]):
                ...
                frame = vid.get_data(match_ts.T[1][frame_num]) #get a frame from top vid  that actually has a corresponding bottom match. 
                if j == 0:
